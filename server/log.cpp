...
void logn(const char *name, const char *fmt, ...) {
    ...
    snprintf(filename, sizeof(filename), "%s/%s.txt", Path::Log.c_str(), name);
    fp = fopen(filename, "a");
...
void loge(const char *fmt, ...) {
    ...
    snprintf(file, sizeof(file), "%s/log.txt", Path::Log.c_str());
...
void loga(const char *fmt,...) {
    ...
    snprintf(file, sizeof(file), "%s/log.active.txt", Path::Log.c_str());
    fd = open(file, O_RDWR, 0);
    if(fd < 0) {
        fd = open(file, O_RDWR | O_CREAT, ACC);
        if(fd < 0)
