...
void init_mudModule(py::module &);
PYBIND11_EMBEDDED_MODULE(mud, mudModule) {
    init_mudModule(mudModule);
}

void init_mudStatModule(py::module &m);
PYBIND11_EMBEDDED_MODULE(mudStat, mudStatModule) {
    init_mudStatModule(mudStatModule);
}
...
//        pybind11_init();
        auto mudModule = py::module_::import("mud");
//        py::scope(mudModule).attr("gConfig") = py::ptr(gConfig); // Make the gConfig object available
//        auto locals = py::dict("gConfig"_a=(gConfig));
...
//        // Setup the std::string --> pythonStr converter
//        boost::python::to_python_converter<std::string, std::stringToPythonStr>();
//
//        // And the pythonStr --> std::string converter
//        std::stringFromPythonStr();
...
//
//        py::object IOModule((py::handle<>(PyImport_ImportModule("io"))));
...
//        py::object mudLibModule((py::handle<>(PyImport_ImportModule("mudLib"))));
//        pythonHandler->mainNamespace["mudLib"] = mudLibModule;
//
//        // Now import the modules we setup above
//        py::object mudModule((py::handle<>(PyImport_ImportModule("mud"))));
//        pythonHandler->mainNamespace["mud"] = mudModule;
//
//        py::object mudObjectsModule(
//                (py::handle<>(PyImport_ImportModule("MudObjects"))));
//        pythonHandler->mainNamespace["MudObjectsSystem"] = mudObjectsModule;
//
//        // Add in any objects we want
//        py::scope(mudModule).attr("gConfig") = py::ptr(gConfig); // Make the gConfig object available
//        py::scope(mudModule).attr("gServer") = py::ptr(gServer); // Make the gServer object available
//
//        // Run a python test command here
//        return runPython("print(\"Python Initialized! Running Version \" + mud.gConfig.getVersion())");
//    }
//    catch (py::error_already_set const &pythonErr) {
//        std::clog << "PythonError: " << pythonErr.what() << std::endl;
    }
    catch (...) {
        PyErr_Print();
        return false;
...
//
////      { //::MudObject::getName
////          typedef std::string& (::MudObject::*getName_function_type)();
////
...
//
//            typedef bool ( ::MudObject::*isEffected_function_type )( ::std::string const & ,bool ) const;
//
...
//        { //::MudObject::addEffect
//
//            typedef ::EffectInfo * ( ::MudObject::*addEffect_function_type )( ::std::string const & ,long int,int,::MudObject *,bool,::Creature const *,bool ) ;
//
//            MudObject_exposer.def(
//                "addEffect"
...
//
//    typedef ::Container * ( ::Container::*findCreature_function_type )( ::Creature *,::std::string const &,bool,bool,bool );
//
...
//
//            typedef ::Creature * ( ::BaseRoom::*findCreature_function_type )( ::Creature *,::std::string const &,bool,bool,bool );
//
//            BaseRoom_exposer.def(
//                    "findCreature"
...
//}
//
//// This handles std::string to PythonString conversions
//struct std::stringToPythonStr {
...
//};
//
//struct std::stringFromPythonStr {
//    std::stringFromPythonStr() {
...
//PYBIND11_EMBEDDED_MODULE(mud, m) {
//    // `m` is a `py::module_` which is used to bind functions and classes
//    m.def("add", [](int i, int j) {
//        return i + j;
//    });
//}
//PYBIND11_EMBEDDED_MODULE_IMPL(mud);
...
//bool addMudObjectToDictionary(bp::object& dictionary, std::string key, MudObject* myObject) {
...
bool Server::runPython(const std::string& pyScript, py::object& localNamespace) {
//    try {
//        // Run a python test command here
//        bp::handle<> ignored((
//
...
bool Server::runPython(const std::string& pyScript, std::string args, MudObject *actor, MudObject *target) {
...
//  actor:      The actor of the script.
//  target:     The target of the script
bool Server::runPythonWithReturn(const std::string& pyScript, std::string args, MudObject *actor, MudObject *target) {
//
//    try {
//
//        bp::object localNamespace((bp::handle<>(PyDict_New())));
...
//            if(returned.check())
//                errText += ": " + returned();https://www.youtube.com/watch?v=orJSJGHjBLI
//            else
